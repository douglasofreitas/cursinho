...
class QuestaoQuestionariosController extends AppController {
	...
	var $paginasQuestionario = array(
        '1' => array('1', '2', '90', '86', '6'),
        '2' => array( '24'),
        '3' => array('34',  '36', '37', '38', '80', '81'),
        '4' => array('41', '42', '43', '44', '87', '49',
            '53', '54', '83', '55', '88', '56', '89', '57', '58', '91', '92', '93'),
        '5' => array('59', '60', '65'),
        '6' => array('67'),
        '7' => array('71', '72',
            '27', '28', '29', '30', '31', '32', '33'));

    var $paginasQuestionario_candidato = array(
        '1' => array('1', '2', '90', '86', '6'),
        '2' => array( '24', '94'),
        '3' => array('34',  '36', '37', '38', '80', '81'),
        '4' => array('41', '42', '43', '44', '87', '49',
             '53', '54', '83', '55', '88', '56', '96', '89', '57', '58', '91', '93', '92'),
        '5' => array('71', '72',
            '27', '28', '29', '30', '31', '32', '33')
		//'5' => array('59', '60'),
        //'6' => array('71', '72',
	...
	function salvar_respostas($numero_inscricao, $ano, $pagina, $novaPagina = null)
	{
        ...
        if($this->Session->read('Auth.User.group_id') == 3){
            $paginasQuestionario = $this->paginasQuestionario_candidato;
        }if($this->Session->read('Auth.User.group_id') == 1){
            $paginasQuestionario = $this->paginasQuestionario;
        }else{
            $paginasQuestionario = $this->paginasQuestionario_candidato;
        }
		...
		{	
			...
			foreach ($paginasQuestionario[$pagina] as $questao_id)
			{
				$resposta = array();
				$etnia_count = 0;
						...
						if ($valor != '')
						{
							switch ($questao_id)
							{
                                ...
                                case '65' :
                                    ...
                                    switch ($campo)
                                    {
                                        case 'qid65_chk_1' : $pendencia = 1;
                                            break;
                                        case 'qid65_chk_2' : $pendencia = 1;
                                            break;
                                        case 'qid65_chk_3' : $pendencia = 1;
                                            break;
                                        case 'qid65_chk_4' : $pendencia = 1;
                                            break;
                                        case 'qid65_chk_5' : $pendencia = 1;
                                            break;
                                        case 'qid65_chk_6' : $pendencia = 1;
                                            break;
                                        case 'qid65_chk_7' : $pendencia = 1;
                                            break;
                                        case 'qid65_chk_8' : $pendencia = 1;
                                            break;
                                    }
            ...
            if(!empty($sexo_candidato))
                $candidato['Candidato']['sexo'] = $sexo_candidato;
            if(!empty($cor_candidato))
                $candidato['Candidato']['cor'] = $cor_candidato;
            if($pendencia === 1 | $pendencia === 0){
                    $candidato['Candidato']['questionario_pendencias'] = $pendencia;
            }
            if(!empty($etnia_candidato[0]))
                $candidato['Candidato']['etnia1'] = $etnia_candidato[0];
            if(!empty($etnia_candidato[1]))
            ...
            if(!empty($_SESSION['Inscricao']['candidato_id'])){
                    ...
                    $this->redirect('/questao_questionarios/preencher_candidato/' . $candidato_id . '/' . $novaPagina);
            }else{
                if($this->Session->read('Auth.User.group_id') == 3){
                    $this->redirect('/questao_questionarios/preencher_candidato/' . $candidato_id . '/' . $novaPagina);
                }else{
    ...
    function preencher_candidato($candidato_id, $pagina)
    {
        //verifica se o candidato é do usuário atual
        $candidatos = $this->Candidato->getByCPF($this->Session->read('Auth.User.username'));
        $pode_ver = false;
        foreach($candidatos as $c){
            if($c['Candidato']['candidato_id'] == $candidato_id){

                //verificar se ainda pode editar, pelo período de inscrição
        ...
        if(!empty($_SESSION['Inscricao']['candidato_id'])){
            if($_SESSION['Inscricao']['candidato_id'] == $candidato_id)
                $pode_ver = true;
        }
        if(!$pode_ver){
            $this->redirect('/candidatos/index');
        ...
        // Verifica se a página fornecida está dentro do limite de páginas do questionário
        if ($pagina <= 0 || $pagina > $this->totalPaginas_candidato)
            $pagina = 1;
        // Obtem o id do candidato informado
        ...
        if ($candidato_id)
        {
            ...
            foreach ($this->paginasQuestionario_candidato[$pagina] as $questao_id)
            {
            ...
            $this->set('numero_inscricao', $candidato['Candidato']['numero_inscricao']);
            // Guardamos o ano fornecido
            $this->set('ano', $candidato['Candidato']['ano']);
            $this->set('candidato_id', $candidato_id );
            // Guardamos o nome do candidato
            $this->set('nome', $this->Candidato->obterNome($candidato['Candidato']['numero_inscricao'], $candidato['Candidato']['ano']));
	...
	function beforeFilter() {
		...
		$this->set('moduloAtual', 'candidatos', 'preencher_candidato');
	}
