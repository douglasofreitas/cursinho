...
<table class="listagem"> 
    <tr class="listagem_header"> 
      <td style="width:60px; text-align:center;"><?php echo $paginator->sort('Ano', 'ano'); ?></td> 
        <td style="width:60px; text-align:center;"><?php echo $paginator->sort('Número inscrição', 'numero_inscricao'); ?></td>
        <td style="width:60px; text-align:center;"><?php echo $paginator->sort('Código do estudante', 'estudante_id'); ?></td>
        ...
        <td style="width:100px; text-align:center;">Data da evasão </td>
    </tr> 
  ...
  <?php $numero_linha = 0; ?> 
    ...
    <?php endif; ?> 
              ...
              <td style="text-align:center;">
                <?php 
                if(count($estudante['Evasao']) > 0){
                    $evasao = $estudante['Evasao'][0];
                    echo $html->link(
                        $evasao['data_form'],
                        '/evasaos/visualizar/' . $estudante['Estudante']['estudante_id'],
                        array('class' => 'tipHoverBottom', 'title' => 'Visualizar evasão')
                    );
                }else{
                    echo 'Não há evasão';
                }
