<?php 
class QuestaoQuestionario extends AppModel {

	var $name = 'QuestaoQuestionario';

	var $useTable = 'questao_questionario';
	var $primaryKey = 'questao_questionario_id';

	var $hasMany = array('RespostaQuestaoQuestionario' => array('className' => 'RespostaQuestaoQuestionario',
																'foreignKey' => 'questao_questionario_id'));

	function obterQuestao($questao_id)
	{
		$condicao = array('questao_questionario_id' => $questao_id);

		if ($this->find('count', array('conditions' => $condicao, 'recursive' => '0')) > 0)
		{
			$questao = $this->find('first', array('conditions' => $condicao, 'recursive' => '0'));
			//return $questao;
			return $questao['QuestaoQuestionario']['questao'];
		}
		else
		{
			return false;
	...
	function montar_questao($questao)
	{
		...
		$htmlQuestao .= '<p class="questionario">' . $numero . $xmlQuestao->texto . '</p>' . "\n";
